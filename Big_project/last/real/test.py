...
def sort_lst(self, lst,column_to_sort,sort,project_name,table_name):
...
class GitDatastoreApiProjects:
    ...
    def update_table(self, table_name: str, row_to_update: int, new_row: dict, branch_name: str):
        ...
        try:
            with open('for_update.jsons', mode='w') as table:
                ...
                gen = (row for row in self._database.files.get(file_path=f'{table_name}.jsons', ref=branch_name).decode().decode(encoding='utf-8').split('\n'))
                i = 0
                for row in gen:
                    i += 1
                    row = str((new_row if i == row_to_update else row)) + '\n'
                    table.write(row)
                if row_to_update > len(list((row for row in self._database.files.get(file_path=f'{table_name}.jsons', ref=branch_name).decode().decode(encoding='utf-8').split('\n')))):
    ...
    def sort_table(self, table_name: str, column_to_sort, type_sort='grow'):
        try:
            gen = (row for row in self._database.files.get(file_path=f'{table_name}.jsons', ref='master').decode().
                decode(encoding='utf-8').split('\n'))
            lst = []
            for row in gen:
                lst.append(json.dumps(row))
            if column_to_sort in self._database.files.get(file_path=f'{table_name}.conf', ref='master').decode().
                decode(encoding='utf-8').split('\n')):
                raise HTTPException(404, "Данный столбец отсутствует в таблице")
            if type_sort == 'grow':
                sorted_lst = sorted(lst, key=itemgetter(column_to_sort))
    ...
    def read_table(self, table_name, branch_name: str, stop=None, start=None, sort=None,column_to_sort=None):
        branch_name = self.branch_verify(branch_name)
        try:
