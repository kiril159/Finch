...
| `final_task/`       | A Python directory that contains code for your Dagster repository                 |
| `final_task_tests/` | A Python directory that contains tests for `final_task`                      |
...
pyenv virtualenv $PYTHON_VERSION final_task
pyenv activate final_task
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name final_task python=PYTHON_VERSION
conda activate final_task
```
...
Tests can be found in `final_task_tests` and are run with the following command:

```bash
pytest final_task_tests
```

As you create Dagster ops and graphs, add tests in `final_task_tests/` to check that your
